...
export interface FaultyTerminalProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  curvature?: number;
  tint?: string;
  backgroundTint?: string; // 【新增】背景颜色控制
  centerVignette?: boolean; // 【新增】中间区域渐变效果
  mouseReact?: boolean;
...
uniform vec3  uTint;
uniform vec3  uBackgroundTint; // 【新增】Shader 接收背景色变量
uniform vec2  uMouse;
...
void main() {
    ...
    // 获取亮度值（这里拿到的是黑白灰度值）
    vec3 brightnessColor = getColor(p);

    if(uChromaticAberration != 0.0){
      vec2 ca = vec2(uChromaticAberration) / iResolution.xy;
      brightnessColor.r = getColor(p + ca).r;
      brightnessColor.b = getColor(p - ca).b;
    }

    // 【核心修改】：使用 mix 混合背景色和前景色
    // brightnessColor.r 代表亮度强度（0.0为背景，1.0为前景）
    // mix(背景色, 前景色, 强度)
    vec3 col = mix(uBackgroundTint, uTint, brightnessColor.r);
...
function hexToRgba(hex: string): [number, number, number, number] {
  ...
  if (h.length === 8) {
    ...
    alpha = parseInt(h.slice(3, 4) + h.slice(3, 4), 16) / 255;
    h = h.slice(0, 3);
  }
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    gl.clearColor(
      backgroundColorVec[0],
      backgroundColorVec[1],
      backgroundColorVec[2],
      backgroundColorVec[3]
    );
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        // 【新增】传入背景色 Uniform
        uBackgroundTint: { value: new Color(backgroundTintVec[0], backgroundTintVec[1], backgroundTintVec[2]) },
        uMouse: {
